...
export async function generateTypes(
  ...
  const schemaNames = spec.components?.schemas ? getSortedSchemas(spec) : [];
  ...
  for (const schemaName of schemaNames) {
    ...
    const outFile = resolve(typesDir, typeFileName(typeName));
  ...
  const indexWriter = fileWriter(resolve(typesDir, "index.ts"));
