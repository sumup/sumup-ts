...
export async function generateResource(
  ...
  const needsRequestOptionGuard = iterPathConfig(spec.paths).some(
    ({ methodSpec, pathSpec, opId }) => {
      if (!methodSpec.tags?.includes(tag.name)) {
        return false;
      }
  ...
  writer.w0("import { APIResource, type APIPromise, ");
  if (needsRequestOptionGuard) {
    writer.w0("isRequestOptions, ");
  }
  writer.w('type RequestOptions } from "../../core";');
  ...
  const renderType = (schema: OpenAPIV3_1.SchemaObject) => {
    let out = "";
    const typeWriter = {
      w: (str: string) => {
        out += `${str}\n`;
    ...
    schemaToTypes(schema, typeWriter);
    return out.trim();
  };
...
export class ${resourceClassName} extends APIResource {`);
  for (const { methodSpec, pathSpec, opId, method, path } of iterPathConfig(
    ...
    const body = getRequestBody(opId, methodSpec);
    const pathSignature = pathParams
      .map((param) => `${Case.camel(param.name)}: ${renderType(param.schema!)}`)
      .join(", ");
    const hasOptionalQuery =
      queryParams.length > 0 && !queryParams.some((param) => param.required);
    const joinSignature = (...parts: (string | null)[]) =>
      parts.filter(Boolean).join(", ");

    if (hasOptionalQuery && !body) {
      writer.w(
        `${methodName}(${joinSignature(
          pathSignature || null,
          `query?: ${queryParamsType(methodNameType)}`,
          "options?: RequestOptions",
        )}): APIPromise<${successType}, ${errorTypeName}>;`,
      );
      writer.w(
        `${methodName}(${joinSignature(
          pathSignature || null,
      ...
      writer.w(
        "  const requestOptions = isRequestOptions(queryOrOptions) ? queryOrOptions : options;",
      );
      writer.w(
